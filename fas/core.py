...
def div_by_threads(dir_list: list) -> list:
    th_list_list = []
...
def do_everything():
    ...
    category_dirs = get_category_pathdirs()
    dirs = [dir for dir in dirs if dir not in category_dirs]

    th_list_list = div_by_threads(dirs)
    for th_list in th_list_list:
        thid_list = []
        for dir in th_list:
            thid = Thread(target=thread_for_path, args=(dir,))
            thid.start()
            thid_list.append(thid)
        [thid.join() for thid in thid_list]
